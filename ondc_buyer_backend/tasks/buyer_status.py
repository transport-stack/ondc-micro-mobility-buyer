...
@shared_task
def buyer_status(cached_on_confirm_data, **kwargs):
    try:
        ...
        logging.info(f"seller_status_payload============: {seller_status_payload}")

        SELLER_APP_SITE_URL = context['bpp_uri'] + '/status'
        post_result = post_request(SELLER_APP_SITE_URL, seller_status_payload)

        status_start_time_cache_key = f"{context['transaction_id']}:{CACHE_DELIMITER}:status_start_time"
        cache.set(status_start_time_cache_key, formatted_current_utc, timeout=TIMESTAMP_CACHE_TIMEOUT)

        return post_result

    except Exception as e:
        logger.error(f"Error in buyer_status: {e}")
        raise
