...
@shared_task
def buyer_cancel(cached_on_confirm_data, reason, **kwargs):
    try:
        ...
        logging.info(f"seller_cancel_payload============: {seller_cancel_payload}")

        SELLER_APP_SITE_URL = context['bpp_uri'] + '/cancel'
        post_result = post_request(SELLER_APP_SITE_URL, seller_cancel_payload)

        cancel_start_time_cache_key = f"{context['transaction_id']}:{CACHE_DELIMITER}:cancel_start_time"
        cache.set(cancel_start_time_cache_key, formatted_current_utc, timeout=TIMESTAMP_CACHE_TIMEOUT)
