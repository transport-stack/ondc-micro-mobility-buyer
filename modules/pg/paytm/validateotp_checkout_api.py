...
def validateotp_checkout_api(transaction_token, otp, order_id):
    paytmParams = dict()

    assert type(otp) is str
    paytmParams["body"] = {"otp": otp}

    paytmParams["head"] = {"txnToken": transaction_token}

    post_data = json.dumps(paytmParams)

    url = "{}/login/validateOtp?mid={}&orderId={}".format(
    ...
    try:
        ...
        logger.error("couldn't connect to paytm server in validateotp_checkout_api")
        raise
